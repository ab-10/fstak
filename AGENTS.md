...
## Project memory: SPX relationship

- `fstak` is currently experimental and developed as a parallel surface to SPX.
- If `fstak` proves successful, the intended direction is to merge it into SPX rather than keep long-term duplicated platform primitives.
- For Neon during the experimental phase, default to a dedicated Neon branch (not a separate long-lived Neon project) to reduce duplication while preserving isolation and safe rollback.

## Project memory: opencode logs

- The opencode session pointer for this repo is stored at `.git/opencode`.
- Local opencode state lives under `~/.local/share/opencode/`; session metadata/messages are in `~/.local/share/opencode/opencode.db`, runtime logs are in `~/.local/share/opencode/log/`, and tool outputs are in `~/.local/share/opencode/tool-output/`.
